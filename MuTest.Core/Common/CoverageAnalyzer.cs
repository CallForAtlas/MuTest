    ...
    public class CoverageAnalyzer : ICoverageAnalyzer
    {
        ...
        public void FindCoverage(SourceClassDetail source, CoverageDS codeCoverage)
        {
            if (codeCoverage != null)
            {
                ...
                if (coverages.Any())
                {
                    source.Coverage = Coverage.Create(
                        (uint)coverages.Sum(x => x.LinesCovered),
                        (uint)coverages.Sum(x => x.LinesNotCovered),
                        (uint)coverages.Sum(x => x.BlocksCovered),
                        (uint)coverages.Sum(x => x.BlocksNotCovered));
                    ...
                    foreach (var coverage in coverages)
                    {
                        ...
                        foreach (CoverageDSPriv.MethodRow mCoverage in methods)
                        {
                            ...
                            if (methodDetail != null)
                            {
                                ...
                                {
                                    methodDetail.Coverage = Coverage.Create(0, 0, 0, 0);
                                }

                                methodDetail.Coverage = Coverage.Create(
                                    methodDetail.Coverage.LinesCovered + mCoverage.LinesCovered,
                                    methodDetail.Coverage.LinesNotCovered + mCoverage.LinesNotCovered,
                                    methodDetail.Coverage.BlocksCovered + mCoverage.BlocksCovered,
                                    methodDetail.Coverage.BlocksNotCovered + mCoverage.BlocksNotCovered);
                            ...
                            if (coverages.Any())
                            {
                                ...
                                var classCoverage = new ClassCoverage
                                {
                                    ...
                                    Coverage = Coverage.Create(
                                        (uint)coverages.Sum(x => x.LinesCovered) - autogeneratedLineCovered,
                                        (uint)coverages.Sum(x => x.LinesNotCovered) - autogeneratedLineNonCovered,
                                        (uint)coverages.Sum(x => x.BlocksCovered) - autogeneratedBlockCovered,
                                        (uint)coverages.Sum(x => x.BlocksNotCovered) - autogeneratedBlockNonCovered),
